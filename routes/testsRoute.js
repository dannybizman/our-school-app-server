...
router.post("/create",  authorizeRoles(["teacher"]), async (req, res) => {
  try {
    ...
    const populated = await Test.findById(saved._id).populate("lessonId");
...
router.get("/:id",  authorizeRoles(["teacher", "student", "parent"]), async (req, res) => {
  try {
    const test = await Test.findById(req.params.id).populate("lessonId");
...
router.put("/update/:id",  authorizeRoles(["teacher"]), async (req, res) => {
  try {
    const updated = await Test.findByIdAndUpdate(req.params.id, req.body, { new: true }).populate("lessonId");
